...
// Тест для проверки исправленной логики NeuralLanguageClassifier

// Имитируем исправленную логику classifyWithHeuristics
func classifyWithHeuristics(_ text: String) -> (language: String?, confidence: Double) {
    ...
    print("Checking impossible patterns (min length: 3)...")
...
print("=== Тест исправленной NeuralLanguageClassifier логики ===")
print("Минимальная длина паттерна: 3 символа")
print()
...
for testWord in testCases {
    ...
    if let detectedLanguage = result.language {
        print("Слово: '\(testWord)' (длина: \(testWord.count))")
        ...
        if testWord.count >= 5 && result.confidence >= 0.9 {
            print("  Переключение: ✅ (confidence \(String(format: "%.2f", result.confidence)) >= 0.9)")
        } else if testWord.count < 5 && result.confidence >= 0.8 {
            print("  Переключение: ✅ (confidence \(String(format: "%.2f", result.confidence)) >= 0.8)")
        } else {
...
// Проверяем, какие паттерны срабатывают
print()
print("=== Проверка паттернов ===")
print()

for pattern in ["рщ", "рщц", "фку", "нщг", "цр", "црфе", "ер", "ерфе", "еру", "ерун", "фтв", "ащк", "цшер"] {
    print("Паттерн '\(pattern)': длина \(pattern.count) символов, >=3? \(pattern.count >= 3 ? "✅" : "❌")")
}