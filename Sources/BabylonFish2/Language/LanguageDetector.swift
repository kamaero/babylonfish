...
class LanguageDetector {
    ...
    func detectLanguage(for keyCodes: [Int]) -> Language? {
        ...
        // 1.1 Проверяем ключевые слова программирования (сильный сигнал)
        if LanguageConstants.programmingKeywords.contains(enLower) {
            logDebug("Programming Keyword Match: \(enLower)")
            return .english
        }
    ...
    func suggestCorrection(for word: String, language: Language) -> String? {
        ...
        if let correction = correction, correction != word {
             // Игнорируем если исправление отличается только регистром (для авто-замены)
             // Если пользователь хочет lower, а система предлагает Capitalized - это ок, но
             // иногда это раздражает. В данном случае оставим как есть.
             
             // Дополнительная проверка: не предлагать слишком короткие исправления
             if correction.count < 3 { return nil }
             
             // Проверка на сильное отличие (Levenshtein) - NSSpellChecker иногда предлагает странное
    ...
    func isSystemWord(_ word: String, language: String) -> Bool {
